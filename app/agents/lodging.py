...
def lodging_node(state: TravelState) -> TravelState:
    ...
    try:
        ...
        return {"search_request": search_request, "lodging_candidates": [], "errors": [str(exc)], "messages": ["숙소 검색 서비스 호출에 실패했습니다."]}
    candidates: list[LodgingCandidate] = [{"place_id": item.place_id, "name": item.name, "distance_km": item.distance_km, "status": item.status.value, "missing_fields": item.missing_fields} for item in response.results]
    return {"search_request": search_request, "lodging_candidates": candidates, "messages": [f"숙소 후보 {len(candidates)}개를 찾았습니다."]}
